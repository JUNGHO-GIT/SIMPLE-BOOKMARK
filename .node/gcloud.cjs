...
const fnDetectOsAndArgs = () => {
	...
	fnLogger(`info`, `운영체제 감지: ${winOrLinux}`);
	fnLogger(`info`, `전달된 인자: ${args.length > 0 ? args.join(', ') : 'none'}`);
...
const fnModifyChangelog = () => {
	fnLogger(`info`, `changelog.md 업데이트 시작`);
	...
	fnLogger(`success`, `changelog.md 업데이트 완료: ${versionArray.join('.')}`);

	return versionArray.join('.');
};

// git push 공통 함수 ---------------------------------------------------------------------------
const fnGitPush = (remoteName=``, ignoreFilePath=``, winOrLinux=``) => {
	fnLogger(`info`, `Git Push 시작: ${remoteName}`);
	...
	fnLogger(`info`, `현재 브랜치: ${currentBranch}`);
	fnLogger(`info`, `.gitignore 파일 교체: ${ignoreFilePath}`);
	...
	statusOutput ? (() => {
		...
		fnLogger(`success`, `커밋 완료`);
	})() : fnLogger(`info`, `변경사항 없음 - 커밋 건너뜀`);

	fnLogger(`info`, `Push 진행: ${remoteName} ${currentBranch}`);
	execSync(`git push --force ${remoteName} ${currentBranch}`, { stdio: 'inherit' });
	fnLogger(`success`, `Push 완료: ${remoteName}`);

	fs.writeFileSync(ignoreFile, ignorePublicFile, 'utf8');
	fnLogger(`info`, `.gitignore 파일 복원`);
};

// 실행 ---------------------------------------------------------------------------------------
(() => {
	fnLogger(`info`, `스크립트 실행: gcloud.cjs`);
	...
	try {
		...
		args.includes(`--full-deploy`) || args.includes(`--only-git`) ? (() => {
			fnModifyChangelog();
			fnGitPush(`origin`, `.gitignore.public`, winOrLinux);
			fnGitPush(`private`, `.gitignore.private`, winOrLinux);
			fnLogger(`success`, `배포 완료`);
		})() : (() => {
