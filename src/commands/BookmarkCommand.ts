...
import { isFileType, LRUCache, logger, notify, validateFileName } from "@exportScripts";
import type { BookmarkModelType, BookmarkProviderType, ExcludeRuleType } from "@exportTypes";

const MAX_EXPLORER_EXPAND_FOLDERS = 250;
const CLIPBOARD_LINE_PATTERN = /\r?\n/;
type ExpandBudget = { count: number; limited: boolean };

// ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――
// 1. 북마크 명령
// ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――
export const BookmarkCommand = (provider: BookmarkProviderType, _context: vscode.ExtensionContext) => {
  // 0. 변수 설정 ―――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
  const excludeRuleCache = new LRUCache<string, ExcludeRuleType[]>(50);
  ...
  // 1-1. when 절 추출
  const getWhenClause = (value: boolean | { when?: string }): string | undefined => {
  ...
  const getRelativePath = (folder: vscode.WorkspaceFolder, target: vscode.Uri): string => {
  ...
  const getExcludeRuleTypesForFolder = (folder: vscode.WorkspaceFolder): ExcludeRuleType[] => {
  ...
  // 조건부 숨김 여부를 결정 ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――--
  const evaluateWhenClause = async (whenClause: string, folder: vscode.WorkspaceFolder, relativePath: string): Promise<boolean> => whenClause.includes("$(basename)") ? await (async () => {
  ...
  // 스킵할지 여부를 판단 ―――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――--
  const shouldSkipEntry = async (uri: vscode.Uri, folderHint?: vscode.WorkspaceFolder, kind?: vscode.FileType): Promise<boolean> => {
  ...
  // 1. 폴더 북마크 재귀 수집 ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
  const collectFolderBookmarks = async (item?: BookmarkModelType, visited: Set<string> = new Set()): Promise<BookmarkModelType[]> => {
  ...
  // 2. 폴더 펼침 상태 일괄 반영 ―――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
  const setFolderExpansionState = async (expanded: boolean): Promise<void> => {
  ...
  const expandAllExplorerFolders = async (): Promise<boolean> => {
    ...
    try {
      ...
      for (const folder of workspaceFolders) {
        await expandFolderRecursively(folder.uri, budget);
        if (budget.limited) {
          break;
        }
      }
  ...
  const expandFolderRecursively = async (folderUri: vscode.Uri, budget: ExpandBudget): Promise<void> => {
    try {
      if (budget.count >= MAX_EXPLORER_EXPAND_FOLDERS) {
        budget.limited = true;
        return;
      }
      budget.count++;
      ...
      for (const subDir of subDirectories) {
        ...
        await expandFolderRecursively(subDir, budget);
      }
    }
    catch (error) {
  ...
  // 선택된 아이템 업데이트 ―――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――--
  const updateSelectedBookmark = (items: BookmarkModelType[]): void => {
  ...
  // URI 기반 북마크 추가 ―――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――--
  const addBookmarkFromUri = async (targetUri: vscode.Uri): Promise<void> => {
  ...
  const registerAddBookmarkCommand = (): vscode.Disposable =>
    vscode.commands.registerCommand("Simple-Bookmark.addbookmark", async (uri?: vscode.Uri) => {
      if (uri) {
        await addBookmarkFromUri(uri);
      }
      else {
        await vscode.commands.executeCommand("copyFilePath");
        const copied = await vscode.env.clipboard.readText();
        const picked = copied ? vscode.Uri.file(copied.split(CLIPBOARD_LINE_PATTERN)[0]) : undefined;

        if (picked) {
          await addBookmarkFromUri(picked);
        }
        else {
      ...
      return itemsToRemove.length === 0 && originalItemsToRemove.length === 0 ? notify(`error`, `remove - 삭제할 북마크가 선택되지 않았습니다.`) : await (async () => {
            ...
            if (itemsToRemove.length > 0) {
              ...
              if (deleteMode === "bookmarkOnly") {
                ...
                const itemCountText = itemsToRemove.length === 1 ? "1 bookmark" : `${itemsToRemove.length} bookmarks`;

                const choice = await vscode.window.showWarningMessage(`How would you like to delete ${itemCountText}?`, { modal: true }, "Bookmark Only", "Bookmark + Original File");
            ...
            if (originalItemsToRemove.length > 0) {
              const itemCountText = originalItemsToRemove.length === 1 ? "1 original item" : `${originalItemsToRemove.length} original items`;
              const choice = await vscode.window.showWarningMessage(`Delete ${itemCountText} inside bookmarked folders?`, { modal: true }, "Delete Original");
            ...
            itemsToRemove.length > 0 && (() => {
                const successMessage = itemsToRemove.length === 1 ? removedBookmarksWithOriginal ? "Bookmark and original file deleted" : "Bookmark deleted" : removedBookmarksWithOriginal ? `${itemsToRemove.length} bookmarks and original files deleted` : `${itemsToRemove.length} bookmarks deleted`;
  ...
  const registerRenameBookmarkCommand = (): vscode.Disposable =>
    vscode.commands.registerCommand("Simple-Bookmark.renamebookmark", async (item?: BookmarkModelType) => {
  ...
  const registerCopyBookmarkCommand = (): vscode.Disposable =>
    vscode.commands.registerCommand("Simple-Bookmark.copybookmark", (item?: BookmarkModelType, selected?: BookmarkModelType[]) => {
      let targets: BookmarkModelType[] = Array.isArray(selected) && selected.length > 0 ? selected : selectedBookmarks.length > 0 ? selectedBookmarks : item ? [item] : [];
  ...
  const registerPasteBookmarkCommand = (): vscode.Disposable =>
    vscode.commands.registerCommand("Simple-Bookmark.pastebookmark", async (item?: BookmarkModelType) => {
  ...
  const registerCreateFolderCommand = (): vscode.Disposable =>
    vscode.commands.registerCommand("Simple-Bookmark.createfolder", async (item?: BookmarkModelType) => {
  ...
  const registerCreateFileCommand = (): vscode.Disposable =>
    vscode.commands.registerCommand("Simple-Bookmark.createfile", async (item?: BookmarkModelType) => {
  ...
  // 3. 선택 폴더 펼침 ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――
  const registerExpandBookmarkFolderCommand = (): vscode.Disposable =>
    vscode.commands.registerCommand("Simple-Bookmark.expandbookmarkfolder", async (item?: BookmarkModelType) => {
      const target: BookmarkModelType | undefined = item || (selectedBookmarks.length > 0 ? selectedBookmarks[0] : undefined);
  ...
  const registerExpandExplorerCommand = (): vscode.Disposable =>
    vscode.commands.registerCommand("Simple-Bookmark.expandexplorer", async () => {
      ...
      return !folders || folders.length === 0 ? notify(`warn`, `select - No workspace folder available to expand.`) : await (async () => {
            ...
            const limited = await expandAllExplorerFolders();
            limited ? notify(`warn`, `expand - Stopped after ${MAX_EXPLORER_EXPAND_FOLDERS} folders to protect VS Code responsiveness.`) : notify(`info`, `select - Explorer expanded for all workspace folders.`);
          })();
    });

  // 특정 폴더 확장 ―――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――
  const registerExpandFolderCommand = (): vscode.Disposable =>
    vscode.commands.registerCommand("Simple-Bookmark.expandfolder", async (uri: vscode.Uri) => {
      ...
      try {
        ...
        logger(`debug`, `expand - ${uri.fsPath}`);
        const budget: ExpandBudget = { count: 0, limited: false };
        await expandFolderRecursively(uri, budget);

        budget.limited ? notify(`warn`, `expand - Stopped after ${MAX_EXPLORER_EXPAND_FOLDERS} folders to protect VS Code responsiveness.`) : notify(`info`, `expand - Expanded: ${path.basename(uri.fsPath)}`);
      }
      catch (error) {
  ...
  return {
    ...
    registerCommands: (): vscode.Disposable[] => [registerRefreshCommand(), registerAddBookmarkCommand(), registerRemoveBookmarkCommand(), registerRenameBookmarkCommand(), registerCopyBookmarkCommand(), registerPasteBookmarkCommand(), registerPasteToRootBookmarkCommand(), registerDeleteAllBookmarkCommand(), registerCreateFolderCommand(), registerCreateFileCommand(), registerExpandBookmarkFolderCommand(), registerExpandAllBookmarksCommand(), registerCollapseAllBookmarksCommand(), registerExpandExplorerCommand(), registerExpandFolderCommand()],
  };
};
