...
export const BookmarkCommand = (
	...
	const getWhenClause = (
		...
		const whenClause =
			typeof value === "object" && value ? value.when : undefined;
	...
	const getExcludeRuleTypesForFolder = (
		...
		return cached
			...
			: (() => {
					...
					const raw =
						config.get<Record<string, boolean | { when?: string }>>(
							"exclude",
						) ?? {};
		...
		return whenClause.includes("$(basename)")
			? await (async () => {
					...
					const siblingRelative =
						directory === "." ? substituted : `${directory}/${substituted}`;
					const segments = siblingRelative
						.split("/")
	...
	const expandAllExplorerFolders = async (): Promise<void> => {
		try {
			...
			logger(
				`debug`,
				`select - ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	};
	...
	const expandFolderRecursively = async (
		...
		try {
			...
			const filteredEntries = await Promise.all(
				entries
					.filter(([_name, type]: [string, vscode.FileType]) =>
						isFileType(type, vscode.FileType.Directory),
					)
			...
			const subDirectories = filteredEntries.filter(
				(uri: vscode.Uri | null) => uri !== null,
			) as vscode.Uri[];
			...
			logger(
				`debug`,
				`expand - ${folderUri.fsPath} ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	};
			...
			async (uri?: vscode.Uri) => {
				return uri
					...
					: await (async () => {
							...
							const picked = copied
								? vscode.Uri.file(copied.split(/\r?\n/)[0])
								: undefined;
							return picked
								? await (async () => {
										...
										return stat.type === vscode.FileType.Directory ||
											...
											: notify(
													`error`,
													`add - Only files or folders can be added.`,
												);
									})()
								: notify(
										`error`,
										`add - No file or folder selected in Explorer.`,
									);
						})();
			},
		);
			...
			async (item?: BookmarkModelType) => {
				...
				for (const candidate of candidateItems) {
					provider.isRootBookmark(candidate.originalPath)
						...
						: candidate.isOriginalAvailable &&
							originalOnlyTargets.add(candidate.originalPath);
				}

				const itemsToRemove: string[] = Array.from(
					rootBookmarkTargets.values(),
				);
				const originalItemsToRemove: string[] = Array.from(
					originalOnlyTargets.values(),
				).filter((targetPath) => {
					return !itemsToRemove.some((rootPath) =>
						isWithinPath(rootPath, targetPath),
					);
				});
				...
				return itemsToRemove.length === 0 && originalItemsToRemove.length === 0
					...
					: await (async () => {
							...
							if (itemsToRemove.length > 0) {
								const config =
									vscode.workspace.getConfiguration("Simple-Bookmark");
							...
							if (originalItemsToRemove.length > 0) {
								const itemCountText =
									originalItemsToRemove.length === 1
										? "1 original item"
							...
							itemsToRemove.length > 0 &&
								(() => {
									const successMessage =
										itemsToRemove.length === 1
											? removedBookmarksWithOriginal
												? "Bookmark and original file deleted"
												: "Bookmark deleted"
											: removedBookmarksWithOriginal
												? `${itemsToRemove.length} bookmarks and original files deleted`
												: `${itemsToRemove.length} bookmarks deleted`;
	...
	const registerRenameBookmarkCommand = (): vscode.Disposable =>
		vscode.commands.registerCommand(
			...
			async (item?: BookmarkModelType) => {
				const target: BookmarkModelType | undefined =
					item ||
					(selectedBookmarks.length > 0 ? selectedBookmarks[0] : undefined);
					...
					: await (async () => {
							...
							return !newName
								...
								: await (async () => {
										...
										notify(
											`info`,
											`rename - Renamed: ${currentName} → ${newName.trim()}`,
										);
									})();
						})();
			},
	...
	const registerCopyBookmarkCommand = (): vscode.Disposable =>
		vscode.commands.registerCommand(
			...
			(item?: BookmarkModelType, selected?: BookmarkModelType[]) => {
				let targets: BookmarkModelType[] =
								...
								: await (async () => {
										const targetPath: string | undefined = item
											? (updateSelectedBookmark([item]),
												!item.bookmarkMetadata.isFile &&
												item.isOriginalAvailable
													? item.originalPath
													: path.dirname(item.originalPath))
											: selectedBookmarks.length > 0
												? (() => {
														const folder = selectedBookmarks.find(
															(s) =>
																!s.bookmarkMetadata.isFile &&
																s.isOriginalAvailable,
														);
														return folder
															? folder.originalPath
															: path.dirname(selectedBookmarks[0].originalPath);
													})()
												: provider.rootPath;
										...
										return targetPath
											...
											: notify(
													`warn`,
													`paste - Select a valid target folder to paste into.`,
												);
									})();
						})();
			},
		...
		vscode.commands.registerCommand(
			...
			async (item?: BookmarkModelType) => {
				const folderName = await vscode.window.showInputBox({
					prompt:
						"[Simple-Bookmark] Enter folder name (will be created in original location)",
		...
		vscode.commands.registerCommand(
			...
			async (item?: BookmarkModelType) => {
				const fileName = await vscode.window.showInputBox({
					prompt:
						"[Simple-Bookmark] Enter file name (will be created in original location)",
					validateInput: validateFileName,
				});
			...
			async (item?: BookmarkModelType) => {
				...
				return !target
					...
					: target.bookmarkMetadata.isFile
						...
						: (provider.markExpanded(target.originalPath), provider.refresh());
			},
		);
			...
			async () => {
				...
				return !folders || folders.length === 0
					...
					: await (async () => {
							...
							notify(
								`info`,
								`select - Explorer expanded for all workspace folders.`,
							);
						})();
			},
		);
			...
			async (uri: vscode.Uri) => {
				...
				try {
					...
					if (!(stat.type & vscode.FileType.Directory)) {
						notify(
							`warn`,
							`select - Selected item is not a folder: ${uri.fsPath}`,
						);
						return;
					}
