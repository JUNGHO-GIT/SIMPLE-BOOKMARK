...
export const createBookmarkCommand = (
	...
	vscode.commands.registerCommand("JEXPLORER.refreshentry", () => {
		console.debug("[JEXPLORER.cmd.refresh]");
		provider.refresh();
	});

	// 북마크 추가 (Explorer 선택 기반) --------------------------------------------------------
	const registerAddBookmarkCommand = (): vscode.Disposable =>
	vscode.commands.registerCommand("JEXPLORER.addbookmark", async (uri?: vscode.Uri) => {
		if (!uri) {
	...
	vscode.commands.registerCommand("JEXPLORER.removebookmark", async (item?: BookmarkSystemItem) => {
	...
	vscode.commands.registerCommand("JEXPLORER.renamebookmark", async (item?: BookmarkSystemItem) => {
	...
	const registerCopyBookmarkCommand = (): vscode.Disposable =>
		vscode.commands.registerCommand("JEXPLORER.copybookmark", (item?: BookmarkSystemItem, selected?: BookmarkSystemItem[]) => {
	...
	vscode.commands.registerCommand("JEXPLORER.pastebookmark", async (item?: BookmarkSystemItem) => {
		...
		if (targetPath) {
			console.debug("[JEXPLORER.pastebookmark]", targetPath);
	...
	vscode.commands.registerCommand("JEXPLORER.pasterootbookmark", async () => {
		if (!provider.hasCopiedItems()) {
			vscode.window.showErrorMessage("No items to paste.");
			return;
		}
	...
	vscode.commands.registerCommand("JEXPLORER.removeallbookmark", async () => {
	...
	vscode.commands.registerCommand("JEXPLORER.createfolder", async (item?: BookmarkSystemItem) => {
	...
	vscode.commands.registerCommand("JEXPLORER.createfile", async (item?: BookmarkSystemItem) => {
