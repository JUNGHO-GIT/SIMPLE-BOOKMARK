...
import type {
	BookmarkMetadata,
	BookmarkModelType,
	BookmarkOperationServiceType,
	BookmarkSyncServiceType,
} from "@exportTypes";
...
export const BookmarkProvider = (workspaceRoot: string | undefined) => {
	...
	const _onDidChangeTreeData = new vscode.EventEmitter<
		BookmarkModelType | undefined | null | void
	>();
	...
	setTimeout(
		() => (
			initializeBookmarkFolder().catch((err: unknown) =>
				logger(
					`error`,
					`activate - ${err instanceof Error ? err.message : String(err)}`,
				),
			),
			0
		),
	...
	const migrateLegacyBookmarkFolder = async (
		legacyPath: string,
		targetPath: string,
	): Promise<void> => {
		...
		try {
			...
			const metadataEntries = entries.filter(([name, type]) => {
				return type === vscode.FileType.File && name.endsWith(`.bookmark.json`);
			});
			...
			for (const [name] of metadataEntries) {
				...
				await vscode.workspace.fs.rename(
					vscode.Uri.file(legacyFilePath),
			...
			migratedCount > 0 &&
				(notify(
					`info`,
					`migrate - ${migratedCount} bookmark metadata moved to ${targetPath}`,
				),
				logger(
					`info`,
					`migrate - ${legacyPath} -> ${targetPath} (${migratedCount})`,
				));
		} catch (error) {
	...
	const initializeBookmarkFolder = async (): Promise<void> => {
		...
		return !hasRoot
			...
			: await (async () => {
					...
					const legacyBookmarkPath = getLegacyBookmarkPath(
					...
					bookmarkPath &&
						...
						(fileOperationService = BookmarkOperationService(
							bookmarkPath,
							syncService,
						)));
				})();
	...
	// TreeItem을 그대로 반환 ―――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
	const getTreeItem = (element: BookmarkModelType): vscode.TreeItem => {
		return element;
	};
				...
				: await (async () => {
						...
						return isCycle
							...
							: !element.bookmarkMetadata.isFile
								? await getFolderContents(element.originalPath, ancestor)
								: [];
					})();
	};
	...
	const getRootBookmarks = async (): Promise<BookmarkModelType[]> => {
		...
		for (const metadata of bookmarks) {
			const status =
				bookmarkStatusMap.get(metadata.originalPath) ?? BookmarkStatus.SYNCED;
		...
		return !ready
			...
			: await (async () => {
					try {
						...
						logger(
							`debug`,
							`remove - ${originalPath} ${deleteOriginal ? "with original" : "bookmark only"}`,
						);
					} catch (error) {
								...
								try {
									...
									const ext =
										stat.type === vscode.FileType.File
											...
											: "";
									const baseCandidate =
										stat.type === vscode.FileType.File && !hasDot
											? `${newName}${ext}`
											: newName;
									...
									const mkUnique = async (
										...
										while (true) {
											...
											if (
												resolvedCandidate === resolvedOriginal ||
												(process.platform === "win32" &&
													resolvedCandidate.toLowerCase() ===
														resolvedOriginal.toLowerCase())
											) {
												return name;
											}
											try {
		...
		return visited.has(normalizedPath)
			...
			: await (async () => {
					try {
						...
						logger(
							`debug`,
							`paste: failed to collect files from ${folderPath} ${error}`,
						);
					}
					return files;
				})();
			...
			: await (async () => {
					...
					for (const m of all) {
						m.isFile
							...
							: await (async () => {
									const folderFiles = await collectFilesFromFolder(
										m.originalPath,
									);

									const isBaseBookmark =
										!m.isFile &&
										(m.bookmarkName === `.base` ||
											path.basename(m.originalPath) === `.base`);
	...
	// 원본 파일/폴더 직접 삭제 ―――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――
	const deleteOriginalItems = async (
		...
		return !ready
			? notify(`error`, `activate - File operation service is not initialized.`)
			: await fileOperationService!.deleteOriginalFiles(
					originalPaths.map((originalPath) => vscode.Uri.file(originalPath)),
				);
	};
