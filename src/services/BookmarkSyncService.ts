...
import { logger, validateFileName } from "@exportScripts";
...
export const BookmarkSyncService = (
		...
		const saveListener = vscode.workspace.onDidSaveTextDocument(
			async (document) => {
				...
				!isBookmarkedFile(filePath) ||
					(logger(`debug`, `save - ${filePath}`), await syncBookmark(filePath));
			},
		);
			...
			for (let i = 0; i < metaPaths.length; i += BATCH_SIZE) {
				...
				const batchPromises = batch.map(async (metadataPath) => {
					try {
						...
						logger(
							`error`,
							`activate - ${error instanceof Error ? error.message : String(error)}`,
						);
						return null;
					}
				});
	...
	const addBookmark = async (
		...
		try {
			...
			const metadataPath = path.join(
				bookmarkPath,
				`${uniqueBookmarkName}${METADATA_EXT}`,
			);
	...
	const renameBookmark = async (
		...
		const finalMetaName = existsOther
			? generateUniqueBookmarkName(newNameRaw)
			: newNameRaw;
		const previousBookmarkName = metadata!.bookmarkName;

		// 실제 파일/폴더 rename 준비
		const dir = path.dirname(metadata!.originalPath);
		const desiredFsName = preserveExt(
		...
		if (
			path.resolve(candidateFsPath) !== path.resolve(metadata!.originalPath)
		) {
			...
			try {
				logger(
					`debug`,
					`rename - ${metadata!.originalPath} -> ${newOriginalPath}`,
				);
		...
		const oldMetaPath = path.join(
			bookmarkPath,
			`${metadata!.bookmarkName}${METADATA_EXT}`,
		);
		const newMetaPath = path.join(
			bookmarkPath,
			`${finalMetaName}${METADATA_EXT}`,
		);
		...
		metadata &&
			(await (async () => {
				try {
					...
					logger(
						`error`,
						`remove: ${error instanceof Error ? error.message : String(error)}`,
					);
				}
			})());
	};
	...
	// 북마크 여부 확인 ―――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
	const isBookmarkedFile = (filePath: string): boolean =>
		bookmarkedFiles.has(filePath);

	// 메타데이터 조회 ―――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――--
	const getAllBookmarks = (): BookmarkMetadata[] =>
		Array.from(bookmarkedFiles.values());

	// 특정 메타데이터 조회 ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――--
	const getBookmark = (originalPath: string): BookmarkMetadata | undefined =>
		bookmarkedFiles.get(originalPath);
